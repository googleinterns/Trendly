...
import {ClusterlyComponent, CLUSTERS_CONTAINER, HEIGHT, Location, Scales, TOOLTIP_CLASS, WIDTH} from './clusterly.component';
...
const CLUSTER_DATA2: ClusterData[] = [
  {title: '', id: 1, queries: [{queryString: '', volume: 1}]}, {
    title: '',
    id: 2,
    queries: [{queryString: '', volume: 1}, {queryString: '', volume: 1}]
  }
];
...
describe('ClusterlyComponent', () => {
  ...
  it('check gridDivision assign valid locations ', () => {
    ...
    for (let i = 1; i <= CLUSTERS_3_GROUPS.size; i++) {
      expect(
          ...
          clusterIdtoLoc.get(i).xPosition <= WIDTH)
          .toBeTrue();
      expect(
          clusterIdtoLoc.get(i).yPosition >= 0 &&
          clusterIdtoLoc.get(i).yPosition <= HEIGHT)
          .toBeTrue();
    }
  });
  ...
  it('check closestGroupId with two valid clusters options', () => {
    ...
    expect((component as any).closestGroupId(50, 50, clusterIdtoLoc)).toBe(1);
  });
