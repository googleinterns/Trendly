...
export class ClusterlyComponent implements OnInit {
  ...
  private addScales(): void {
    ...
    this.scales.set(Scales.RadiusScale, d3.scaleSqrt()
      .domain([1, 100])
      .range([(WIDTH + HEIGHT) / 120, (WIDTH + HEIGHT) / 50]));

    // A scale that gives a color for each bubble
    this.scales.set(Scales.ColorScale, d3.scaleOrdinal()
    ...
    this.scales.set(Scales.LightColorScale, d3.scaleOrdinal()
      .domain(Array.from(this.clusters.keys()))
      .range(this.colorsService.lightColorShow));

    // A scale of the x position for each group
    this.scales.set(Scales.XPositionSacle, d3.scaleLinear()
      .domain([1, this.clusters.size])
      .range([WIDTH / 6, 5 * WIDTH / 6]));
  }
  ...
  private processClustersObjects(clustersData: ClusterData[]): void {
    clustersData.forEach((cluster) => {
      const newCluster: Cluster = new Cluster(cluster.title, cluster.id,
        cluster.queries);
  ...
  private addCircles(circleGroup, id: string, radiusAddition: number,
    ...
    return circleGroup.selectAll('g')
      ...
      .attr('r', (d) => this.scales.get(Scales.RadiusScale)(d.volume)
        + radiusAddition)
  ...
  private closestGroupId(x: number, y: number,
    clusterIdToLoc: Map<number, Location>): number {
    let minDistance: number = Infinity;
    let closestGroupId: number;
    let currDistance: number;

    for (let i = 1; i <= this.clusters.size; i++) {
      currDistance = Math.sqrt(Math.pow(clusterIdToLoc.get(i).xPosition - x, 2)
  ...
  private addForceSimulation(clusterIdToLoc: Map<number, Location>) {
    return d3.forceSimulation()
      .force('x', d3.forceX().strength(0.5)
        .x((d) => clusterIdToLoc.get(d.clusterId).xPosition))
      .force('y', d3.forceY().strength(0.5)
        .y((d) => clusterIdToLoc.get(d.clusterId).yPosition))
      .force('center', d3.forceCenter()
        .x(WIDTH / 2).y(HEIGHT / 2)) // Attraction to the center of the svg
      .force('charge', d3.forceManyBody()
        .strength(1)) // Nodes are attracted one each other
      .force('collide', d3.forceCollide().strength(1)
        .radius((d) => this.scales.get(Scales.RadiusScale)(d.volume) + 1)
